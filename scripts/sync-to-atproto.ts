...
const CONFIG = {
	// Your site URL
	siteUrl: "https://andrewhudson.dev",
...
async function sync() {
	...
	try {
		...
		// Create lookup maps
		const existingByPath = new Map(existingDocs.map((d) => [d.value.path, d]));
		...
		for (const post of posts) {
			...
			// Skip future posts
			if (new Date(post.frontmatter.date) > new Date()) {
				console.log(`   ⏭️  ${post.slug} (future post, skipping)`);
				results.push({ slug: post.slug, action: "skipped" });
				continue;
			}
